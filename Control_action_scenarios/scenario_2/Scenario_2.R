...
population$I_swi_pop[population$node== 25516] <- 1
...
model_1     <- SEIR_model(population = population,                                                 #  Population database
                          ...
                          simulation_name = "scenario_2_init",                                     #  Simulation tag name
                          days_of_simulation = 12,                                                 #  Population database
...
zones_arond_inft_farms <- assign_control_zones(population = population,                      # Population database
                                               ...
                                               num_threads = 2)         # Computer threads to be used (please don't overload your computer)
...
#                                                                          #
#==========================================================================#
initial_day_control <- 13                                                                              # Seed the initial day of infection in the day 21
days_of_control_action <- 90                                                                           # Select 30 days working on control actions
days_to_test <- seq(initial_day_control, (initial_day_control+days_of_control_action), by= 5)          # Create the data that will be used to test and update the buffer control areas
detectection_rate <- 80                                                                                # percentage of detection of the infectious farms
...
for (day in initial_day_control:(initial_day_control+days_of_control_action)) {
  ...
  population_vac <- output_vacc_pop[[1]]
  population_vac <- population_vac[, c(names(population))]
  population <- update_population_in_model(original_population = population,
                                           model_population =  population_vac)
  #=======================================#
  ##             SEIR dynamics        ----
  #=======================================#
  ...
  model_in_loop     <- SEIR_model(population = population,                                                 #  Population database
                                  events = events,                                                         #  Events database
                                  simulation_name = "scenario_2_control",                                  #  Simulation tag name
...
plot_contol_actns <- plot_epi_curve_mean_and_cntrl_act(model_inital = model_1,
                                                       ...
                                                       control_action_start_day = initial_day_control,
